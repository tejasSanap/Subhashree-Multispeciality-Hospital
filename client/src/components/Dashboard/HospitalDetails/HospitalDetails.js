...
const HospitalDetails = () => {
    ...
    const handleSave = () => {
        ...
        axios.put(`/api/hospital/updateHospital/${hospital._id}`, formData)
            .then((response) => {
                setEditing(false);
            })
            .catch((error) => {
        ...
        <Card>
            <Card.Body>
                {editing ? (
                    ...
                    <div>
                        <Card.Title>{hospital?.address}</Card.Title>
                        <Card.Subtitle className="mb-2 text-muted">Monday to Friday: {hospital?.mondayToFriday}</Card.Subtitle>
                        <Card.Subtitle className="mb-2 text-muted">Saturday: {hospital?.saturday}</Card.Subtitle>
                        <Card.Subtitle className="mb-2 text-muted">Sunday: {hospital?.sunday}</Card.Subtitle>
                        <Card.Text>Awards Won: {hospital?.awardsWon}</Card.Text>
                        <Card.Text>Satisfied Patients: {hospital?.satisfiedPatient}</Card.Text>
