...
function Appointment() {
    ...
    const fetchAppointment = async () => {
        if (role == 'doctor') {
            ...
            const filteredAppointments = res.data.filter((appt) => appt.doctorId === doctor._id);
    ...
    const handleApproveAppointment = async (appointmentId) => {
        try {
            const res = await axios.put(`/api/appointment/updateappointment/${appointmentId}`, {
                status: 'confirmed'
            });
            fetchAppointment();
        } catch (error) {
                ...
                <tbody>
                    {myAppointment.map((appointment, index) => (
                        <tr key={index}>
                            ...
                            <td>
                                <Button variant="primary" onClick={() => handleApproveAppointment(appointment._id)}>Approve</Button>
                            </td>
                        </tr>
                    ))}
                </tbody>
